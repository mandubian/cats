...
object FreeView {
  ...
  case class Impure[S[_], A](a: S[FreeR0[S, A]]) extends FreeView[S, A]
}


import FreeR0._

case class FreeR0[S[_], A] private(head: FreeView[S, Val], tail: Ops = Vector.empty) {
  import Op._
  import Val._
  import FreeView._

  def map[B](f: A => B) = FreeR0.map(this)(f)
  def flatMap[B](f: A => FreeR0[S, B]) = FreeR0.bind(this)(f)

  final def resume(implicit S: Functor[S]): (Either[S[FreeR0[S, A]], A]) = FreeR0.toView(this) match {
   ...
   */
  final def go(f: S[FreeR0[S, A]] => FreeR0[S, A])(implicit S: Functor[S]): A = {
    @tailrec def loop(t: FreeR0[S, A]): A =
      t.resume match {
   ...
   * from `S` to a monad `M`.
   */
  final def runM[M[_]](f: S[FreeR0[S, A]] => M[FreeR0[S, A]])(implicit S: Functor[S], M: Monad[M]): M[A] = {
    def runM2(t: FreeR0[S, A]): M[A] = t.resume match {
   ...
   */
  final def mapSuspension[T[_]](f: S ~> T)(implicit S: Functor[S], T: Functor[T]): FreeR0[T, A] =
    this.resume match {
      case Left(s)  => fromView(Impure(f(S.map(s)(((_: FreeR0[S, A]) mapSuspension f)))))
      case Right(r) => fromView(Pure(r))
    }

  final def compile[T[_]](f: S ~> T)(implicit S: Functor[S], T: Functor[T]): FreeR0[T, A] = mapSuspension(f)
}
...
object FreeR0 {
  ...
  implicit def FreeR0Monad[S[_]] = new Monad[({ type l[A] = FreeR0[S, A] })#l] {
    def pure[A](a: A): FreeR0[S, A] = FreeR0.pure(a)
    def flatMap[A, B](fa: FreeR0[S, A])(f: A => FreeR0[S, B]): FreeR0[S, B] = FreeR0.bind(fa)(f)
  }

  final def pure[S[_], A](a: A): FreeR0[S, A] = FreeR0(Pure[S, Val](cast(a)))

  final def suspend[S[_], A](s: S[FreeR0[S, A]]): FreeR0[S, A] = FreeR0(Impure[S, Val](castK2[FreeR0, S, A](s)))

  final def bind[S[_], A, B](free: FreeR0[S, A])(f: A => FreeR0[S, B]): FreeR0[S, B] = {
    FreeR0(
      free.head,
      free.tail :+ Bind(x => castK1[FreeR0, S, B](f(reify[A](x))))
    )
  }

  final def map[S[_], A, B](free: FreeR0[S, A])(f: A => B): FreeR0[S, B] =
    FreeR0(free.head, free.tail :+ Map(x => f(reify[A](x))))

  /** Suspends a value within a functor lifting it to a Free */
  final def liftF[F[_], A](value: => F[A])(implicit F: Functor[F]): FreeR0[F, A] =
    suspend[F, A](F.map(value)(pure _))

  final def fromView[S[_], A](h: FreeView[S, A]): FreeR0[S, A] = FreeR0(castK1(h))

  @tailrec
  final def toView[S[_], A](h: FreeR0[S, A])(implicit sf: Functor[S]): FreeView[S, A] = {
    h.head match {
      case Pure(x) =>
        h.tail match {
          case Vector() => Pure(reify[A](x))
          case fh +: tailOps => fh match {
            case Map(fmap) => toView(FreeR0(Pure[S, Val](fmap(x)), tailOps))
            case Bind(bind) =>
              val fm = bind(x)
              toView(FreeR0(castK11[FreeView, S, Val](fm.head), fm.tail ++ tailOps))
          }  
        }

      case Impure(f) =>
        Impure(sf.map(f){ fm =>
          FreeR0(fm.head, fm.tail ++ h.tail)
        })
    }
  }

  def mapFusion[S[_], A](h: FreeR0[S, A])(implicit sf: Functor[S]): FreeR0[S, A] =
    h.head match {
      case Pure(_) => FreeR0(h.head, Ops.mapFusion(h.tail))

      case Impure(f) => FreeR0(Impure(sf.map(f){
        fm => FreeR0(fm.head, Ops.mapFusion(fm.tail))
  ...
  object Op {
    ...
    case class Bind[S[_]](f: Val => FreeR0[S, Val]) extends Op
    // case class Apply(f: (Val, Val) => Val, left: Thunk, right: Thunk) extends Op
  }

  import cats.std.function._
  type Trampoline[A] = FreeR0[Function0, A]
