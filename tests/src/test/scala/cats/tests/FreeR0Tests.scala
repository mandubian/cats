...
/*
  test("FreeR0 Monoid is a list") {
    type FreeMonoid[A] = FreeR0[({type λ[+α] = (A,α)})#λ, Unit]
    ...
    object ListF {
      def cons[A](a: A): FreeMonoid[A] = FreeR0.suspend[({type λ[+α] = (A,α)})#λ, Unit]((a, FreeR0.pure[({type λ[+α] = (A,α)})#λ, Unit](())))
  ...
  test("FreeR0 Trampoline is stacksafe") {
...
*/
  // test("FreeR0 should support left binds") {
  //   assert(testTime2("lftBindR 10000000")(lftBind(10000000)(gen _).run) == 10000000)
  // }

  test("FreeR0 should support right binds") {
