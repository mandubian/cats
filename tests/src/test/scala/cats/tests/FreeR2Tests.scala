...
class FreeR2Tests extends CatsSuite {
  ...
  test("FreeR2 Monoid is a list") {
    type FreeMonoid[A] = FreeR2[({type λ[+α] = (A,α)})#λ, Unit]
    ...
    object ListF {
      def cons[A](a: A): FreeMonoid[A] = FreeR2.suspend[({type λ[+α] = (A,α)})#λ, Unit]((a, FreeR2.pure[({type λ[+α] = (A,α)})#λ, Unit](())))
  ...
  test("FreeR2 Trampoline is stacksafe") {
  ...
  // test("FreeR2 should support left binds") {
  //   assert(testTime2("lftBindR 10000000")(lftBind(10000000)(gen _).run) == 10000000)
  // }

  test("FreeR2 should support right binds") {
